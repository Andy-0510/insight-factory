...
def call_gemini_for_weekly_summary(context):
    ...
    2. **[INSIGHT]**: 이 흐름이 **디스플레이 제조기업의** 비즈니스에 주는 기회 또는 위협 요소를 분석해주세요.
...
def call_gemini_for_theme_description(theme_name, keywords):
    ...
    이 테마가 현재 시장에서 어떤 의미를 가지는지 **디스플레이 제조 기업** 입장에서 **1~2 문장**으로 간결하게 설명해주세요.
    ### 테마 설명 (1~2 문장):
    """
...
def call_gemini_for_competitor_insight(competitor_name, mentions, momentum):
    """LLM을 호출하여 주요 기업의 주간 활동에 대한 해설을 생성합니다."""
    prompt = f"""
    시장 분석가로서, 주요 기업 '{competitor_name}'의 지난 주 활동 데이터는 다음과 같습니다:
    - 주간 총 언급량: {mentions}
    - 주간 평균 모멘텀 (z-like): {momentum:.2f}

    이 데이터와 기업에 대한 학습 정보를 바탕으로 '{competitor_name}'의 **이번 주 핵심 활동**과 **주목해야 할 전략적 움직임**을 **디스플레이 제조 기업 관점**의 **한 문장**으로 이해하기 쉽고 간결하게 요약해주세요.
    ### 분석 요약 (한 문장):
    """
    # _call_gemini_safe 함수는 이미 정의되어 있다고 가정
...
def call_gemini_for_weekly_insight(weak_signals: list) -> dict:
    ...
    try:
        ...
        ### 분석 요청:
        목록에 있는 각 '신호'에 대해, **디스플레이 제조 기업의 관점**으로 아래 3가지 항목을 분석하여 JSON 형식으로만 답변해주세요.
...
def call_gemini_for_momentum_recommendation(term, z_like_score, change_display):
    ...
    prompt = f"""
    디스플레이 제조 기업의 시장 분석가로서, '{term}' 키워드의 주간 모멘텀 변화에 대한 **실행 중심의 단기 권고사항**을 **디스플레이 제조 기업의 관점**에서 작성해주세요.
...
def call_gemini_for_portfolio_actions(momentum_summary):
    ...
    이 정보를 바탕으로, **기술/제품 포트폴리오** 관점에서 고려해야 할 **전략적 시사점** 또는 **조정 방향**에 대한 단기적 권고 사항 **2가지**를 **디스플레이 제조 기업의 관점**에서 제안해주세요. (각 제안은 한 문장)
    ### 포트폴리오 권고 (2가지):
        - 
        - 
    """
